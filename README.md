...
`USPSBatchRegistration-Windows-x64` Artifact。解压后运行：

```text
USPSBatchRegistration\USPSBatchRegistration.exe
```
...
3. 点击“生成 CSV 模板”，填写每一行。`email` 留空会自动生成唯一邮箱。
4. 点击“导入 CSV”，确认表格中的邮箱、用户名和账号类型。
5. 默认使用可见浏览器。只有确认当前网络环境支持时才勾选“后台浏览器”。
6. 点击“开始全部”。已成功行和需要人工核对的提交不会重复执行；单条失败不终止后续任务。
7. 点击“导出结果”。默认不导出密码和安全问题答案；勾选敏感凭据后会再次确认。
...
```text
account_type,email,username,password,first_name,last_name,company,address1,address2,
city,state,zip_code,phone,security_answer1,security_answer2
```
